 ...
 * Can be used for debugging, or if \ref reuse_q_file is given to restore the values in the next run.
 *
 */
 ...
 *
 * Every action that would lead to a state outside of the state array (consisting of core and uncore)
 * is set to NaN. The middle field is initialised with a negative value, to prevent the algorithm from
 * getting stuck in the current state. So each state is explored at least once.
 *
...
/** Initialises the last and current state respective last action maps.
 *
